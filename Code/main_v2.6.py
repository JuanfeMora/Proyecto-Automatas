...
class TensorModel:
    ...
    def update_step(self):
        ...
        # CORRECCIÓN DE BUG: La Convección térmica (Térmicas) solo redirige flujos existentes
        hot  = (self.temperature >  5) & air
        cold = (self.temperature < -5) & air
        prob_conv = np.random.random((self.w, self.h)) < 0.3
        
        swap_up = hot & prob_conv & (self.wind[DIR_DOWN] > 0)
        self.wind[DIR_DOWN][swap_up] = np.clip(self.wind[DIR_DOWN][swap_up] - 1, 0, 3)
        self.wind[DIR_UP][swap_up]   = np.clip(self.wind[DIR_UP][swap_up]   + 1, 0, 3)

        swap_down = cold & prob_conv & (self.wind[DIR_UP] > 0)
        self.wind[DIR_UP][swap_down]   = np.clip(self.wind[DIR_UP][swap_down]   - 1, 0, 3)
        self.wind[DIR_DOWN][swap_down] = np.clip(self.wind[DIR_DOWN][swap_down] + 1, 0, 3)
