...
class TensorModel:
    def __init__(self, w, h):
        ...
        # FIX: Inicializar bordes como muros sólidos para evitar wrap-around
    ...
    def reset_all(self):
        ...
        self.temperature.fill(0)
        self.tracers.fill(0)
        # Sellar bordes nuevamente al limpiar todo
        self.materials[0, :] = MAT_WALL
        self.materials[-1, :] = MAT_WALL
        self.materials[:, 0] = MAT_WALL
        self.materials[:, -1] = MAT_WALL
    ...
    def update_step(self):
        ...
        # FIX: Rebote seguro en muros evitando sobrescritura secuencial
        ...
        self.wind = np.clip(new_wind, 0, 3)
        ...
        # FIX: Colisiones seguras evitando referencias encadenadas de NumPy
        air   = (self.materials == MAT_AIR)
        ...
        # Solo celdas de aire participan; muros actúan como barreras térmicas
        T = self.temperature.copy()
        T_air = T * air  # temperatura solo en aire (muros = 0 a efectos de flujo)
        T_adv = T.copy()
        weight = 0.25
        # ch 0→DIR_UP (shift -1 axis 1), ch 1→DIR_DOWN (shift +1 axis 1),
        # ch 2→DIR_LEFT (shift -1 axis 0), ch 3→DIR_RIGHT (shift +1 axis 0)
        for ch, (shift, axis) in enumerate([(-1, 1), (1, 1), (-1, 0), (1, 0)]):
            intensity = self.wind[ch].astype(np.float32) / 3.0
            # La celda vecina trae su temperatura ponderada por la intensidad
            # de viento que la empuja hacia aquí
            src = np.roll(T_air * intensity, shift=shift, axis=axis)
            T_adv += src * weight
            T_adv -= T_air * intensity * weight

        # Difusión (laplaciano) con máscara de aire para no atravesar muros
        T_n = np.roll(T_air, -1, axis=0); T_s = np.roll(T_air, 1, axis=0)
        T_e = np.roll(T_air, -1, axis=1); T_w = np.roll(T_air,  1, axis=1)
        # Contar vecinos que son aire para normalizar el laplaciano
        air_n = np.roll(air.astype(np.float32), -1, axis=0)
        air_s = np.roll(air.astype(np.float32),  1, axis=0)
        air_e = np.roll(air.astype(np.float32), -1, axis=1)
        air_w = np.roll(air.astype(np.float32),  1, axis=1)
        neighbor_sum = T_n * air_n + T_s * air_s + T_e * air_e + T_w * air_w
        neighbor_cnt = air_n + air_s + air_e + air_w
        lap = np.where(neighbor_cnt > 0, neighbor_sum - neighbor_cnt * T_air, 0.0)

        T_new = T_adv + 0.04 * lap
        T_new *= 0.97  # disipación térmica leve
        self.temperature[air] = np.clip(T_new[air], -100.0, 100.0)
        # Muros no acumulan temperatura
        self.temperature[~air] = 0.0

        # FIX: Conveccion conservativa. La temperatura solo redistribuye impulso
        # si ya existe viento en la celda; no crea flujo nuevo desde cero.
        ...
        hot_swap  = (self.temperature >  5) & air & prob_hot  & (self.wind[DIR_DOWN] > 0) & (self.wind[DIR_UP] < 3)
        cold_swap = (self.temperature < -5) & air & prob_cold & (self.wind[DIR_UP] > 0) & (self.wind[DIR_DOWN] < 3)

        self.wind[DIR_DOWN][hot_swap]  = np.clip(self.wind[DIR_DOWN][hot_swap]  - 1, 0, 3)
        self.wind[DIR_UP][hot_swap]    = np.clip(self.wind[DIR_UP][hot_swap]    + 1, 0, 3)

        self.wind[DIR_UP][cold_swap]   = np.clip(self.wind[DIR_UP][cold_swap]   - 1, 0, 3)
        self.wind[DIR_DOWN][cold_swap] = np.clip(self.wind[DIR_DOWN][cold_swap] + 1, 0, 3)
        ...
        # FIX: Destrucción general de trazadores
        new_tracers[self.materials != MAT_AIR] = 0 
    ...
    def render(self, surface):
        ...
        # Trazadores (pequeños cuadrados amarillos)
...
class SimulationGUI:
    ...
    def draw_hud(self, surface):
        ...
        for tool, rect in self.buttons.items():
            ...
            elif active: col = C_BTN_ON
            elif hover: col = (50, 62, 78)
            else: col = C_BTN_OFF
...
def main():
    ...
    pygame.display.set_caption("Lattice Gas Automata  v3 (Final)")
